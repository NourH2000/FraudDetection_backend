...
router.get("/OneQuantityHistoryByID/:idEntrainement", (req, res) => {
  ...
  const query =
    "SELECT * FROM Quantity_result WHERE  id_entrainement= ?  ALLOW FILTERING ";
  client
...
router.get("/OneQuantityHistoryByNumEnr/:NumEnR", (req, res) => {
  ...
  const query =
    "SELECT * FROM Quantity_result WHERE  num_enr= ?  ALLOW FILTERING ";
  client
    .execute(query, [NumEnR], { prepare: true })
    .then((result) => {
      console.log(result);
...
router.get(
  "/OneQuantityHistoryByNumEnr/:NumEnR/:idEntrainement",
  (req, res) => {
    const NumEnR = req.params.NumEnR;
    const idEntrainement = req.params.idEntrainement;
...
router.get("/OneQuantityHistoryByDate/:date", (req, res) => {
  ...
  const query =
    "SELECT * FROM Quantity_result WHERE  date_entrainement= ?  ALLOW FILTERING ";
  client
    .execute(query, [date], { prepare: true })
    .then((result) => {
      console.log(result);
...
router.get("/QuantityResultGroupedByNumEnr", (req, res) => {
  const query =
    "select * from Quantity_result group by num_enr ALLOW FILTERING";
  try {
    client.execute(query, function (err, result) {
...
router.get("/QuantityResultGroupedByNumEnrAndId/", (req, res) => {
  const query =
